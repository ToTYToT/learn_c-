...
class String
{
	...
	public:
		...
		friend ostream & operator<<(ostream & os, const CharProxy & rhs);//读操作
	private:
		int _idx;
		String & _str;
	};
    //friend ostream & operator<<(ostream & os, const CharProxy & rhs);//read 
...
{
	os << rhs._str._pstr[rhs._idx];// 既要访问CharProxy的私有成员，也要访问String的私有成员
	return os;
}
#if 0
